export function buildWhatsAppMessage(
  ...
  items.forEach((item, idx) => {
    ...
    message += `• ${item.name} x${item.quantity} - $${formatNumber(unitPrice, 'es-CO')}%0A`;
  ...
  if (purchaseData.payment_amount) {
    message += `Pago con: $${formatNumber(purchaseData.payment_amount, 'es-CO')}%0A`;
  }
  if (purchaseData.full_name) message += `Nombre: ${purchaseData.full_name}%0A`;
  if (purchaseData.direction) message += `Dirección: ${purchaseData.direction}%0A`;
  if (purchaseData.neighborhood) message += `Barrio: ${purchaseData.neighborhood}%0A`;
